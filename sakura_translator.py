...
class SakuraTranslator():
    ...
    def detect_and_caculate_repeats(self, s: str, threshold: int = _REPEAT_DETECT_THRESHOLD, remove_all=True) -> Tuple[
        bool, str, int, str]:
        """
    ...
    def _check_translation_quality(self, queries: List[str], response: str) -> List[str]:
        ...
        if self._detect_repeats(response, actual_threshold):
            response = _retry_translation(queries, lambda r: self._detect_repeats(r, actual_threshold),
                                          f'检测到大量重复内容（当前阈值：{actual_threshold}），疑似模型退化，重新翻译。')
            if response is None:
    ...
    def _detect_repeats(self, text: str, threshold: int = _REPEAT_DETECT_THRESHOLD) -> bool:
        ...
        检测文本中是否存在重复模式。
        """
        is_repeated, text, count, pattern, actual_threshold = self.detect_and_caculate_repeats(text, threshold,
                                                                                               remove_all=False)
        return is_repeated
    ...
    def _delete_reduntant_ending(self, texts: List[str]) -> List[str]:
        """
        删除文本中的多余结尾符号。
        """
        new_texts = []
    ...
    def _translate(self, from_lang: str, to_lang: str, queries: List[str]) -> List[str]:
        ...
        return self._delete_reduntant_ending(translations)

    def _handle_translation_request(self, prompt: str) -> str:
        """
        处理翻译请求,包括错误处理和重试逻辑。
        """
        retry_attempt = 0
    ...
    def _request_translation(self, input_text_list) -> str:
        """
        向Sakura API发送翻译请求。
        """
